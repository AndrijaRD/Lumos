...
void GUI::Circle(const SDL_Point &center, const int &radius,
                 const SDL_Color &color, const int &thickness) {
  SDL_SetRenderDrawColor(Sys::renderer, color.r, color.g, color.b, color.a);

  // Choose an appropriate number of segments. More segments produce a smoother
  // circle. Here we use at least 32 segments, or more if the circle is large.
  ...
  if (thickness == -1) {
    ...
    // Center vertex:
    SDL_Vertex centerV;
    centerV.position = {static_cast<float>(center.x),
    ...
    SDL_RenderGeometry(Sys::renderer, nullptr, vertices.data(),
    ...
    std::vector<SDL_Vertex> vertices;
    std::vector<int> indices;
    // For each segment, form a quad between outerVerts[i] to outerVerts[i+1]
    // and innerVerts[i] to innerVerts[i+1]
    for (int i = 0; i < segments; i++) {
