...
#define AUTO_ID std::string( string(__FILE__) + ":" + to_string(__LINE__) )
...
private:
     ...
     * and whenever user requests to render a text, first the GUI::Text function
     ...
     * fontSize and color of the text. If there are no matching ids, the new 
     * LoadedText object is pushed into the unordered map, if the unordered map
     * has length greater then MAX_LOADED_TEXTS, the texture that was the least
     * used gets removed. To determene which one is the least used, library
...
public:
    ...
    /** GUI Image
     * 
     * This function renders a Texture.
     * For styling it supports:
     *      - pushBorderRadius
     * 
     * @param td TextureDataPtr, texture to be rendered
