 ...
 * @return 0 on success and positive on error, coresponding to ERROR DEFINITIONS
 */
int Sys::initFont(const string& _fontPath){
    fontPath = _fontPath;
    ...
    if (!FcConfigAppFontAddFile(fcConfig, <const FcChar8*>(fontPath.c_str()))) {
        SDL_Log("Fontconfig failed to load font: %s", fontPath);
        return SYS_FONT_INIT_ERROR;
    }
