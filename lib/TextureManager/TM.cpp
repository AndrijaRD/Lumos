...
// We use the already included nanosvg.h in the header
...
void TextureData::reloadInfo() {
  if (!dptr_->texture) {
    ...
    dptr_->access = (SDL_TextureAccess)SDL_GetNumberProperty(
        prop, SDL_PROP_TEXTURE_ACCESS_NUMBER, 0);

    dptr_->format = (SDL_PixelFormat)SDL_GetNumberProperty(
        prop, SDL_PROP_TEXTURE_FORMAT_NUMBER, 0);
  }
}
...
void TM::removeTexture(std::shared_ptr<TextureData::Impl> dead) {
  ...
  v.erase(std::remove_if(v.begin(), v.end(),
                         [&](auto &wk) {
                           auto sp = wk.lock();
                           // remove if expired OR matches the one we’re
                           // removing:
                           return !sp || sp == dead;
                         }),
          v.end());
...
static int ensureSurfaceFormat(SDL_Surface *&surface) {
...
static int rotateSurface(SDL_Surface *&src, int angle) {
  if (!src)
  ...
  if (!dst)
    return TM_SURFACE_CREATE_ERROR;
...
SDL_Texture *SVGIcon::loadSize(int size) {
  ...
  if (!SDL_GetMasksForPixelFormat(SDL_PIXELFORMAT_RGBA8888, &bpp, &rmask,
                                  &gmask, &bmask, &amask)) {
  ...
  for (int y = 0; y < size; ++y) {
    for (int x = 0; x < size; ++x) {
      ...
      uint32_t pixel = ((uint32_t)rp << rshift) | ((uint32_t)gp << gshift) |
                       ((uint32_t)bp << bshift) | ((uint32_t)a << ashift);
      pixels[y * size + x] = pixel;
    }
  }

  // Create texture and upload (pitch in bytes)
  SDL_Texture *tex = SDL_CreateTexture(Sys::renderer, SDL_PIXELFORMAT_RGBA8888,
...
int TM::loadTexture(TextureData &td, const string &path, const string &id) {
  ...
  td.path = path;
  if (id.empty())
    td.id = fs::path(path).filename().string();
  else
    td.id = id;
...
int TM::loadSVG(SVGIcon &svgIcon, const string &path) {
...
int TM::createTextTexture(TextureData &td, const string &text, int fontSize,
                          int /*weight*/, SDL_Color color) {
  ...
  if (!font) {
    return SYS_FONT_NOT_INITED; // Or appropriate error
  }

  // Render Text
  SDL_Surface *surface = TTF_RenderText_Blended(font, text.c_str(), 0, color);
  // SDL3 TTF_RenderText_Blended takes (font, text, length, color). 0 length
  // means null-terminated? SDL3 migration guide says:
  // TTF_RenderText_Blended(font, text, length, color). If length is 0, it might
  // use strlen? Or I should pass text.length(). Checking SDL3_ttf docs is hard.
  // I'll pass text.length(). Wait, SDL2 was (font, text, color). SDL3 added
  // length.
  ...
  // Convert to Texture
  SDL_Texture *tex = SDL_CreateTextureFromSurface(Sys::renderer, surface);
  if (!tex) {
    SDL_DestroySurface(surface);
  ...
  // Fill other metadata
  td.path = "";
  td.id = text; // Or some ID

  return NO_ERROR;
}

int TM::copyTexture(const TextureData &src, TextureData &dst) {
  ...
  SDL_Texture *newTex =
      SDL_CreateTexture(Sys::renderer, src.getFormat(), src.getAccess(),
  ...
  err = SDL_RenderTexture(Sys::renderer, src.getTexture(), nullptr, nullptr);
  if (!err) {
...
int TM::resizeTexture(const TextureData &src, TextureData &dst, int &newWidth,
                      int &newHeight) {
  if (!src.getTexture())
    return INVALID_ARGUMENTS_PASSED;
  ...
  SDL_Texture *newTex = SDL_CreateTexture(Sys::renderer, src.getFormat(),
                                          src.getAccess(), newWidth, newHeight);
  if (!newTex)
    return TM_TEXTURE_CREATE_ERROR;
  ...
  SDL_FRect dstRect = {0, 0, static_cast<float>(newWidth),
                       static_cast<float>(newHeight)};
  err = SDL_RenderTexture(Sys::renderer, src.getTexture(), nullptr, &dstRect);
  if (!err) {
    SDL_SetRenderTarget(Sys::renderer, old_renderTarget);
    SDL_DestroyTexture(newTex);
    return TM_RCPY_FAILED; // Render Copy FAILED
...
// OVERLOAD
int TM::resizeTexture(const TextureData &src, TextureData &dst,
...
int TM::rotateTexture(const TextureData &src, TextureData &dst, int angle) {
  // 1) Validate inputs
...
int TM::cropTexture(const TextureData &src, TextureData &dst, SDL_Rect rect) {
  // 1) Validate inputs
  SDL_Texture *srcTex = src.getTexture();
  if (!srcTex || !Sys::renderer) {
    return INVALID_ARGUMENTS_PASSED;
  ...
  if (rect.x < 0 || rect.y < 0 || rect.x + rect.w > src.getWidth() ||
      rect.y + rect.h > src.getHeight() || rect.w <= 0 || rect.h <= 0) {
  ...
  SDL_Texture *newTex =
      SDL_CreateTexture(Sys::renderer, src.getFormat(),
                        TextureData::defaultAccess, rect.w, rect.h);
  if (!newTex)
    return TM_TEXTURE_CREATE_ERROR;
  ...
  SDL_FRect srcF = {float(rect.x), float(rect.y), float(rect.w), float(rect.h)};
  SDL_FRect dstF = {0.0f, 0.0f, float(rect.w), float(rect.h)};

  // Copy that region (no rotation, no flip)
  SDL_RenderTextureRotated(Sys::renderer, srcTex, &srcF, &dstF,
                           0.0,     // angle
...
int TM::transformTexture(const TextureData &src, TextureData &dst,
  ...
  SDL_Surface *surf;
  if (int ec = TM::convert_textureTo(src, surf))
    return ec;
  if (int ec = ensureSurfaceFormat(surf))
    return ec;
...
int TM::exportTexture(const std::string &path, const TextureData &td) {
  if (!td.getTexture() || !Sys::renderer)
    return INVALID_ARGUMENTS_PASSED;
  return exportTexture(path, td.getTexture());
}

int TM::exportTexture(const std::string &path, SDL_Texture *tex) {
  if (tex == nullptr || !Sys::renderer)
    return -1;
...
int TM::exportSurface(const std::string &path, SDL_Surface *surface) {
...
int TM::convert_toTexture(const cv::Mat &cvMat, TextureData &td) {
  ...
  bool err = SDL_UpdateTexture(tex,
                               nullptr, // entire texture
                               cvMat.data, static_cast<int>(cvMat.step));
...
int TM::convert_toTexture(const SDL_Surface *surface, TextureData &td) {
  td.setTexture(nullptr);
...
int TM::convert_toTexture(const SDL_Surface *surface, SDL_Texture *&tex) {
  // Create SDL_Texture*
  // ------------------------------------------------------------------------
  tex = SDL_CreateTexture(Sys::renderer, TextureData::defaultPixelFormat,
                          TextureData::defaultAccess, surface->w, surface->h);
  if (tex == nullptr)
    return TM_TEXTURE_CREATE_ERROR;
...
int TM::convert_textureTo(const TextureData &td, SDL_Surface *&surface) {
...
int TM::convert_textureTo(SDL_Texture *tex, SDL_Surface *&surface) {
  // 1) Remember old render‐target, switch to the texture we want to read
  SDL_Texture *oldTarget = SDL_GetRenderTarget(Sys::renderer);
