...
void GUI::Image(SDL_Texture *texture, SDL_Rect &dr_org) {
  // ===== CREATE FRECT ===== ===== =====
  SDL_FRect dr = {static_cast<float>(int(std::round(dr_org.x))),
  ...
  int texWidth =
      (int)SDL_GetNumberProperty(prop, SDL_PROP_TEXTURE_WIDTH_NUMBER, 0);
  int texHeight =
      (int)SDL_GetNumberProperty(prop, SDL_PROP_TEXTURE_HEIGHT_NUMBER, 0);

  // ===== UPDATE MISSING DIMENSIONS ===== ===== =====
  if (dr.w == -1)
    dr.w = dr.h * (float)texWidth / texHeight;
  if (dr.h == -1)
    dr.h = dr.w * (float)texHeight / texWidth;
  ...
  container->contentHeight =
      std::max(dr_org.y + dr_org.h, container->contentHeight);
  ...
  if (dr.y < container->scrollOffset &&
    ...
    SDL_FRect absoluteRect = {static_cast<float>(dr.x + container->dRect.x),
    ...
    SDL_FRect srcRect = {
        static_cast<float>(0),
        static_cast<float>(texHeight * static_cast<float>(invisiblePart) /
                           dr.h),
  ...
  if (dr.y < bottomEdge && (dr.y + dr.h) > bottomEdge) {
    float visiblePart = static_cast<float>(bottomEdge - dr.y);

    SDL_FRect absoluteRect = {
        static_cast<float>(dr.x + container->dRect.x),
        static_cast<float>(dr.y - container->scrollOffset + container->dRect.y),
        static_cast<float>(dr.w), visiblePart};

    SDL_FRect srcRect = {0, 0, static_cast<float>(texWidth),
...
void GUI::Image(TextureData &td, SDL_Rect &dr_org) {
  Image(td.getTexture(), dr_org);
}

void GUI::Image(SVGIcon &icon, SDL_Rect &dRect) {
