...
void GUI::beginContainer(const string &containerId, const SDL_Rect &rect) {
...
void GUI::endContainer() {
  ...
  renderVerticalScrollbar(state->dRect, state->contentHeight,
                          state->scrollOffset);
...
void GUI::renderVerticalScrollbar(
  ...
  const SDL_FRect track = {float(container.x + container.w - barW),
                           float(container.y), barW, float(container.h)};
  ...
  SDL_Point m = Input::Mouse::GetAbsolutePos();
...
bool GUI::isRectVisible(SDL_Rect rect) {
  if (rect.x < 0 && rect.x + rect.w < 0)
    return false;
  if (rect.y < 0 && rect.y + rect.y < 0)
    return false;

  if (activeContainer.empty()) {
    if (rect.x > Sys::wWidth && rect.x + rect.w > Sys::wWidth)
      return false;
    // Sys::winWidth -> Sys::wWidth? Check SM.h
    // SM.h has: inline static int wWidth = 0;

    if (rect.y > Sys::wHeight && rect.y + rect.h > Sys::wHeight)
      return false;
    ...
    if (rect.x > container.dRect.w && rect.x + rect.w > container.dRect.w)
      return false;

    if (rect.y < container.scrollOffset &&
        rect.y + rect.h < container.scrollOffset)
      return false;

    if (rect.y > container.scrollOffset + container.dRect.h &&
