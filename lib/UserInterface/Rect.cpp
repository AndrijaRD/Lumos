 ...
 */
void drawRoundedFilledRect(SDL_Rect rect, unsigned int radius, SDL_Color color,
                           int segments = 12) {
  ...
  SDL_FColor fcol = {color.r / 255.0f, color.g / 255.0f, color.b / 255.0f,
                     color.a / 255.0f};

  // 1) Fill center rectangle
  SDL_FRect r = {float(rect.x + radius), float(rect.y),
                 float(rect.w - 2 * radius), float(rect.h)};
  ...
  r = {float(rect.x), float(rect.y + radius), float(radius),
       float(rect.h - 2 * radius)};
  SDL_RenderFillRect(Sys::renderer, &r);

  r = {float(rect.x + rect.w - radius), float(rect.y + radius), float(radius),
       float(rect.h - 2 * radius)};
  SDL_RenderFillRect(Sys::renderer, &r);
  ...
  auto drawCorner = [&](float cx, float cy, float startDeg) {
    ...
    for (int i = 0; i <= segments; ++i) {
      ...
      verts.push_back(
          {{cx + radius * std::cos(ang), cy + radius * std::sin(ang)},
           fcol,
           {0, 0}});
    }
    ...
    SDL_RenderGeometry(Sys::renderer,
                       /*texture=*/nullptr, verts.data(), (int)verts.size(),
...
computeRoundedRectPolygonFixed(const SDL_Rect &rect, const BorderRadiusRect &br,
                               const SDL_Color &color, int segments = 8) {
  ...
  auto arcTL = makeArc(rect.x + br.top_left, rect.y + br.top_left, 180, 270,
                       br.top_left);
  auto arcTR = makeArc(rect.x + rect.w - br.top_right, rect.y + br.top_right,
                       270, 360, br.top_right);
  auto arcBR =
      makeArc(rect.x + rect.w - br.bottom_right,
              rect.y + rect.h - br.bottom_right, 0, 90, br.bottom_right);
  auto arcBL =
      makeArc(rect.x + br.bottom_left, rect.y + rect.h - br.bottom_left, 90,
              180, br.bottom_left);
  ...
  for (int i = 1; i < (int)arcTR.size(); ++i)
    pushV(arcTR[i].x, arcTR[i].y);
  // 3) arcBR, skip its first
  for (int i = 1; i < (int)arcBR.size(); ++i)
    pushV(arcBR[i].x, arcBR[i].y);
  // 4) arcBL, skip its first
  for (int i = 1; i < (int)arcBL.size(); ++i)
    pushV(arcBL[i].x, arcBL[i].y);

  return vertices;
}

// Helper: Linear interpolation along a polyline.
SDL_FPoint interpolatePoint(const std::vector<SDL_FPoint> &poly,
                            const std::vector<float> &cumLengths, float d) {
  size_t i = 0;
...
void drawDashedLineRect(const SDL_Point &start, const SDL_Point &end,
  ...
  for (int N = 2; N <= maxN; N++) {
    ...
    if (candidateGap < 0)
      continue; // Not possible if dashes exceed L.
  ...
  for (int i = 0; i < bestN; i++) {
    ...
    SDL_Point segStart = {start.x + static_cast<int>(ux * dashStart),
                          start.y + static_cast<int>(uy * dashStart)};
    SDL_Point segEnd = {start.x + static_cast<int>(ux * dashEnd),
                        start.y + static_cast<int>(uy * dashEnd)};
    drawThickLineSegment(segStart, segEnd, color, thickness);
  }
}

void drawRoundedOutlineRect(const SDL_Rect &dRect, const int thickness,
                            const BorderRadiusRect &borderRadius,
                            const SDL_Color &color, const int arcSegments) {
  // Outer polygon (using full radii).
  std::vector<SDL_Vertex> outerVerts =
      computeRoundedRectPolygonFixed(dRect, borderRadius, color, arcSegments);
  ...
  innerBR.top_left = (borderRadius.top_left > (unsigned)thickness)
                         ? borderRadius.top_left - thickness
                         : 0;
  innerBR.top_right = (borderRadius.top_right > (unsigned)thickness)
                          ? borderRadius.top_right - thickness
                          : 0;
  innerBR.bottom_right = (borderRadius.bottom_right > (unsigned)thickness)
                             ? borderRadius.bottom_right - thickness
                             : 0;
  innerBR.bottom_left = (borderRadius.bottom_left > (unsigned)thickness)
                            ? borderRadius.bottom_left - thickness
                            : 0;

  // Using our fixed polygon generator ensures that even when a radius becomes
  // 0, the inner polygon has the same vertex count as the outer polygon.
  std::vector<SDL_Vertex> innerVerts =
      computeRoundedRectPolygonFixed(innerRect, innerBR, color, arcSegments);
  ...
  SDL_RenderGeometry(Sys::renderer, nullptr, borderVerts.data(),
                     static_cast<int>(borderVerts.size()), indices.data(),
                     static_cast<int>(indices.size()));
}

std::vector<SDL_FPoint> computeCornerArc(float cx, float cy, float startAngle,
                                         float endAngle, unsigned int r,
                                         int segments = 8) {
  std::vector<SDL_FPoint> pts;
  for (int i = 0; i <= segments; i++) {
...
void drawDashedEdge(const SDL_Point &start, const SDL_Point &end,
                    const SDL_Color &color, int thickness, int dashSize,
                    int gapSize) {
  float dx = static_cast<float>(end.x - start.x);
  float dy = static_cast<float>(end.y - start.y);
  float L = std::sqrt(dx * dx + dy * dy);
...
void drawRoundedCorners(const SDL_Rect &dRect, const BorderRadiusRect &br,
                        const SDL_Color &color, int thickness,
                        int arcSegments = 8) {
  // Top-left corner
  {
    ...
    std::vector<SDL_FPoint> arc =
        computeCornerArc(cx, cy, 180.f, 270.f, br.top_left, arcSegments);
    for (size_t i = 0; i < arc.size() - 1; i++) {
      SDL_Point p1 = {static_cast<int>(arc[i].x), static_cast<int>(arc[i].y)};
      SDL_Point p2 = {static_cast<int>(arc[i + 1].x),
                      static_cast<int>(arc[i + 1].y)};
      drawThickLineSegment(p1, p2, color, thickness);
    }
  }
  // Top-right corner
  {
    ...
    std::vector<SDL_FPoint> arc =
        computeCornerArc(cx, cy, 270.f, 360.f, br.top_right, arcSegments);
    for (size_t i = 0; i < arc.size() - 1; i++) {
      SDL_Point p1 = {static_cast<int>(arc[i].x), static_cast<int>(arc[i].y)};
      SDL_Point p2 = {static_cast<int>(arc[i + 1].x),
                      static_cast<int>(arc[i + 1].y)};
  ...
  // Bottom-right corner
  {
    ...
    std::vector<SDL_FPoint> arc =
        computeCornerArc(cx, cy, 0.f, 90.f, br.bottom_right, arcSegments);
    for (size_t i = 0; i < arc.size() - 1; i++) {
      SDL_Point p1 = {static_cast<int>(arc[i].x), static_cast<int>(arc[i].y)};
      SDL_Point p2 = {static_cast<int>(arc[i + 1].x),
                      static_cast<int>(arc[i + 1].y)};
  ...
  // Bottom-left corner
  {
    ...
    std::vector<SDL_FPoint> arc =
        computeCornerArc(cx, cy, 90.f, 180.f, br.bottom_left, arcSegments);
    for (size_t i = 0; i < arc.size() - 1; i++) {
      SDL_Point p1 = {static_cast<int>(arc[i].x), static_cast<int>(arc[i].y)};
      SDL_Point p2 = {static_cast<int>(arc[i + 1].x),
                      static_cast<int>(arc[i + 1].y)};
...
void GUI::renderRect(const SDL_Rect &dRect, const SDL_Color &color,
                     const int thickness,
                     const BorderRadiusRect &borderRadius) {
  // ----------------------------------------------------------------------------------------
  // UNDASHED RECT
  ...
  bool noRounded =
      (borderRadius.top_left == 0 && borderRadius.top_right == 0 &&
  ...
  if (!pDashLine) {
    // The function will detect if the borderRadius is 0, so it wont round the
    // corners
  ...
  if (noRounded) {
    ...
    drawDashedLineRect(topLeft, topRight, color, thickness, pDashSize,
                       pDashGapSize);
    drawDashedLineRect(topRight, bottomRight, color, thickness, pDashSize,
                       pDashGapSize);
    drawDashedLineRect(bottomRight, bottomLeft, color, thickness, pDashSize,
                       pDashGapSize);
    drawDashedLineRect(bottomLeft, topLeft, color, thickness, pDashSize,
                       pDashGapSize);
    ...
    cornerRect = {static_cast<float>(dRect.x - fillerSize / 2 - offset),
    ...
    cornerRect = {
        ...
        static_cast<float>(dRect.y - fillerSize / 2 - offset),
        static_cast<float>(fillerSize), static_cast<float>(fillerSize)};
    SDL_RenderFillRect(Sys::renderer, &cornerRect);
  ...
  std::vector<SDL_FPoint> arcTL = computeCornerArc(
      dRect.x + borderRadius.top_left, dRect.y + borderRadius.top_left, 180.f,
      270.f, borderRadius.top_left, arcSegments);
  std::vector<SDL_FPoint> arcTR =
      computeCornerArc(dRect.x + dRect.w - borderRadius.top_right,
                       dRect.y + borderRadius.top_right, 270.f, 360.f,
                       borderRadius.top_right, arcSegments);
  std::vector<SDL_FPoint> arcBR =
      computeCornerArc(dRect.x + dRect.w - borderRadius.bottom_right,
                       dRect.y + dRect.h - borderRadius.bottom_right, 0.f, 90.f,
                       borderRadius.bottom_right, arcSegments);
  std::vector<SDL_FPoint> arcBL =
      computeCornerArc(dRect.x + borderRadius.bottom_left,
                       dRect.y + dRect.h - borderRadius.bottom_left, 90.f,
                       180.f, borderRadius.bottom_left, arcSegments);

  // The dash edges are drawn from:
  // Top edge: from arcTL's last point to arcTR's first point.
  SDL_Point topEdgeStart = {static_cast<int>(arcTL.back().x),
                            static_cast<int>(arcTL.back().y)};
  SDL_Point topEdgeEnd = {static_cast<int>(arcTR.front().x),
                          static_cast<int>(arcTR.front().y)};

  // Right edge: from arcTR's last point to arcBR's first point.
  SDL_Point rightEdgeStart = {static_cast<int>(arcTR.back().x),
                              static_cast<int>(arcTR.back().y)};
  SDL_Point rightEdgeEnd = {static_cast<int>(arcBR.front().x),
                            static_cast<int>(arcBR.front().y)};

  // Bottom edge: from arcBR's last point to arcBL's first point.
  SDL_Point bottomEdgeStart = {static_cast<int>(arcBR.back().x),
                               static_cast<int>(arcBR.back().y)};
  SDL_Point bottomEdgeEnd = {static_cast<int>(arcBL.front().x),
                             static_cast<int>(arcBL.front().y)};

  // Left edge: from arcBL's last point to arcTL's first point.
  SDL_Point leftEdgeStart = {static_cast<int>(arcBL.back().x),
                             static_cast<int>(arcBL.back().y)};
  SDL_Point leftEdgeEnd = {static_cast<int>(arcTL.front().x),
                           static_cast<int>(arcTL.front().y)};

  // Draw dashed lines on each straight edge.
  drawDashedEdge(topEdgeStart, topEdgeEnd, color, thickness, pDashSize,
                 pDashGapSize);
  drawDashedEdge(rightEdgeStart, rightEdgeEnd, color, thickness, pDashSize,
                 pDashGapSize);
  drawDashedEdge(bottomEdgeStart, bottomEdgeEnd, color, thickness, pDashSize,
                 pDashGapSize);
  drawDashedEdge(leftEdgeStart, leftEdgeEnd, color, thickness, pDashSize,
                 pDashGapSize);
}

void GUI::Rect(const SDL_Rect &dRect, const SDL_Color &color,
               const int thickness) {
  ...
  TextureData canvas;
  SDL_Texture *tex =
      SDL_CreateTexture(Sys::renderer, SDL_PIXELFORMAT_RGBA32,
                        SDL_TEXTUREACCESS_TARGET, dRect.w, dRect.h);
  if (tex == nullptr) {
