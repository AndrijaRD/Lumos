...
#define VERSION 3.0 // New version
...
inline std::ostream &operator<<(std::ostream &os, const dateT &date) {
  ...
  oss << std::setw(4) << std::setfill('0') << date.year << "-" << std::setw(2)
...
inline std::ostream &operator<<(std::ostream &os, const timeT time) {
  std::ostringstream oss;
  oss << std::setw(2) << std::setfill('0') << time.hour << ":" << std::setw(2)
      << std::setfill('0') << time.minute << ":" << std::setw(2)
      << std::setfill('0') << time.second;
  os << oss.str();
  return os;
}
...
inline timeT string2time(const string &timeStr) {
  timeT time;
  sscanf(timeStr.c_str(), "%2u:%2u:%2u", &time.hour, &time.minute,
         &time.second);
  return time;
}
...
inline string sdlColor2hex(SDL_Color color) {
  ...
  oss << '#' << std::hex << std::setfill('0') << std::setw(2)
      ...
      << std::setw(2) << static_cast<int>(color.b) << std::setw(2)
...
inline SDL_Color hex2sdlColor(string hex) {
  ...
  auto parsePair = [&](int idx) -> Uint8 {
    return static_cast<Uint8>((hexToByte(hex[idx]) << 4) |
                              hexToByte(hex[idx + 1]));
  };
...
inline SDL_Rect operator+(const SDL_Rect &rect, const SDL_Point &point) {
  return SDL_Rect{rect.x + point.x, rect.y + point.y, rect.w, rect.h};
}

// Scaling the Rect (multiplication)
inline SDL_Rect operator*(const SDL_Rect &rect, float scale) {
  return SDL_Rect{
      static_cast<int>(rect.x * scale), static_cast<int>(rect.y * scale),
 ...
 */
enum ColorShiftMode { SET, SCALE, OFFSET };
...
// DEBUG ENUM
// --------------------------------------------------------------------
enum DebugLevels { All, Warnings, Errors, Silent };

#define TO_FRECT(r)                                                            \
  SDL_FRect { float((r).x), float((r).y), float((r).w), float((r).h) }
...
inline bool isPointInRect(SDL_Point point, SDL_Rect rect) {
