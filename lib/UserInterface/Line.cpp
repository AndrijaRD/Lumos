...
void GUI::Line(const SDL_Point &p1, const SDL_Point &p2, const SDL_Color &color,
  ...
  while (currentDistance < totalLength) {
    // Determine the dash segment: start at currentDistance, dash until either
    // dashSize or the end.
    float dashStart = currentDistance;
    float dashEnd = std::min(currentDistance + pDashSize, totalLength);

    // Compute dash segment endpoints.
    SDL_Point dashP1 = {static_cast<int>(p1.x + ux * dashStart),
                        static_cast<int>(p1.y + uy * dashStart)};
    SDL_Point dashP2 = {static_cast<int>(p1.x + ux * dashEnd),
